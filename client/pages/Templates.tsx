...
// NOTE: Assuming these imports and utilities are correctly implemented
...
import { data } from '@/lib/data'; // Utility for fetching data/profiles
import { supabase } from '@/lib/supabase'; // Supabase client

// Dialog component imports (already present)
...
  // --- New State for handling general errors (e.g., Validation Errors) ---
...
    // Using custom dialog instead of window.confirm
    setErrorTitle('Confirm Clear Data');
    setErrorMessage('Are you sure you want to clear all data? This action cannot be undone.');
    setShowError(true);
  };
...
  const templates = [
    // ... templates definition (unchanged)
    {
...
      let plan = profileData?.plan || 'Free';

      if (!profileData || profileError) {
        // If profile doesn't exist, create it (assuming default is 'Free')
...
        .from('resumes')
        .select('*', { count: 'exact', head: true }) // optimized count
        .eq('user_id', user.id);

      if (countError) console.error("Error fetching resume count:", countError);

      // If Free user and already has 1 or more resumes, block building
      if (plan === 'Free' && (resumeCount || 0) >= 1) {
        setShowUpgrade(true);
        return;
      }

    } catch (error) {
      console.error("Error in template selection check:", error);
      setErrorTitle('Data Fetch Error');
      setErrorMessage('Could not verify your subscription status. Please check your connection.');
      setShowError(true);
      return;
    }
...
  /**
   * Handles PDF generation, checks user plan for download permission,
   * and saves the resume data to Supabase after successful generation.
   */
...
    try {
      // 1. Fetch current plan and resume count
...
      // 2. Check Free user download limit (Blocks if Free user has already 1 resume)
      if (plan === 'Free' && resumeCount >= 1) {
        // Block download if free limit reached
        setShowUpgrade(true);
        return;
      }
...
        setErrorTitle('Validation Required');
        setErrorMessage(`Please fix the following errors before downloading:\n\n- ${validation.errors.join('\n- ')}`);
        setShowError(true);
        return;
      }

      // 4. PDF Generation (This is where the download is triggered)
...
      });
    
      // 5. --- SUCCESS: Save resume data to Supabase (CRUCIAL STEP) ---
      // This ensures the first free download is tracked, and Pro user history is maintained.
      
...
          data: JSON.stringify(resumeData), // ResumeData object must be stored as JSON string
...
        // Note: Download was successful, but history save failed. Not critical enough to stop the user.
      }

    } catch (error) {
      console.error('Error generating PDF:', error);

      // Check for specific Upgrade error thrown by pdfGenerator utility
      if ((error as any)?.code === 'UPGRADE_REQUIRED' || error instanceof UpgradeRequiredError) {
        setShowUpgrade(true);
        return;
      }
      
      // General PDF failure error
      setErrorTitle('Download Failed');
      setErrorMessage('Failed to generate PDF. Check if the template is fully rendered and try again. Technical Error: ' + (error as Error)?.message);
      setShowError(true);
    } finally {
...
      </Dialog>
    
      {/* --- NEW: Generic Error/Confirmation Dialog (replaces alert/confirm) --- */}
...
          </DialogHeader>
          <DialogFooter className="flex gap-2 sm:justify-end">
            {errorTitle === 'Confirm Clear Data' ? (
              <>
                <Button variant="outline" onClick={() => setShowError(false)}>Cancel</Button>
                <Button variant="destructive" onClick={confirmClearData}>Clear Data</Button>
              </>
            ) : (
